...
void queueMultiCommand(client *c, uint64_t cmd_flags) {
    ...
    *mc = head->value;
    head->value = NULL;
    (*mc)->flags |= PENDING_CMD_FLAG_MULTI;

    /* if it's still blocked on keys, count it so that we know when to unblock */
    if ((*mc)->cmd_io_keys_waiting && dictSize((*mc)->cmd_io_keys_waiting))
        c->mstate.num_commands_with_io_blocked_keys++;
