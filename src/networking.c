...
void setDeferredReply(client *c, void *node, const char *s, size_t length) {
    ...
    if (ln->prev != NULL && (prev = listNodeValue(ln->prev)) &&
        prev->size != prev->used && c->io_write_state != CLIENT_PENDING_IO)
    {
...
void unlinkClient(client *c) {
     ...
     * remove it from the list ofprocessIOThreadsReadDone unblocked clients. */
...
parseResult handleParseResults(client *c) {
    ...
    if (c->read_flags & READ_FLAGS_INLINE_ZERO_QUERY_LEN && getClientType(c) == CLIENT_TYPE_SLAVE) {
...
int processClientIOWriteDone(client *c) {
    ...
    if (c->flags & CLIENT_PROTECTED) return 0;
...
int canParseCommand(client *c) {
    ...
    if (c->flags & CLIENT_BLOCKED || c->flags & CLIENT_UNBLOCKED) return 0;

    /* Don't process more buffers from clients that have already pending
     * commands to execute in c->argv. */
    if (c->flags & CLIENT_PENDING_COMMAND) return 0;
    ...
    if (isInsideYieldingLongCommand() && c->flags & CLIENT_MASTER) return 0;
    ...
    if (c->flags & CLIENT_CLOSE_AFTER_REPLY || c->flags & CLIENT_CLOSE_ASAP) return 0;
...
int processInputBuffer(client *c) {
    ...
    while(c->qb_pos < sdslen(c->querybuf)) {
        ...
        c->read_flags = c->flags & CLIENT_MASTER ? READ_FLAGS_PRIMARY : 0;
...
void readToQueryBuf(client *c) {
    ...
    if (!is_primary && // master client's querybuf can grow greedy.
