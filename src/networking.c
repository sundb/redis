...
client *createClient(connection *conn) {
    ...
    c->reply_hold_chunks = NULL;
    c->reply_hold_mem = 0;
    c->reply_hold_boundary_node = NULL;
    c->reply_hold_pending_node = NULL;
    c->reply_hold_pre_command_repl_offset = 0;
...
void installClientWriteHandler(client *c) {
     ...
     * outside c->reply (reply_hold_chunks) until fsynced_reploff covers its
...
static void _addReplyPayloadToList(client *c, list *reply_list, const char *payload, size_t len, uint8_t payload_type) {
    ...
    if (unlikely(reply_list == c->reply && ln == c->reply_hold_boundary_node)) {
        tail = NULL;
    }
...
void updateClientUnsharedReplyBytes(client *c) {
    ...
    /* Scan blocks parked in reply_hold_chunks chunks (appendfsync bgalways):
...
int writeToClient(client *c, int handler_installed) {
    ...
    if (!clientHasPendingReplies(c)) {
         ...
         * moved out to a chunk by replyHoldFinish(), not that nothing
         * remains to send). freeClientAsync() -> freeClient() would tear the
         * chunk down via freeReplyHoldChunks() without ever sending it.
         * drainReplyHoldChunks() re-arms the write handler once the chunk
         * actually drains, so this check will run again then. */
        if ((c->flags & CLIENT_CLOSE_AFTER_REPLY) &&
            !(c->reply_hold_chunks && listLength(c->reply_hold_chunks) > 0))
        {
            freeClientAsync(c);
            return C_ERR;
        }
...
/* ----------------------------------------------------------------------------
 * Reply holding: per-command reply chunking until the data is durable.
 *
 * Engaged by `appendfsync bgalways`: the AOF fsync runs on the bio thread, so a
 * command's reply must not reach the socket until server.fsynced_reploff covers
 ...
 * parked on c->reply_hold_chunks, and spliced back once the durable offset
 ...
 * those bytes are moved OUT of c->reply_bytes into c->reply_hold_mem;
 ...
 * Engaged by appendfsync bgalways. Guarded by aof_enabled (no AOF => nothing to
 * wait for, so we never hang) and by fsynced_reploff != -1 (do not engage during
 * the initial AOF rewrite, where the offset is pinned at -1 and every client
 * would otherwise be held until the rewrite completes). Shared by
 * clientReplyHoldActive and drainReplyHoldChunks so the two stay in lockstep. */
 ...
 * - Engages if EITHER holding is currently enabled by config, OR the client
 *   still has pending chunks from earlier commands. The second condition
 *   preserves RESP reply ordering: once a chunk is parked, any subsequent reply
 ...
 *   command order. */
static int clientReplyHoldActive(client *c) {
    if (c->conn == NULL) return 0;
    if (c->flags & (CLIENT_SLAVE | CLIENT_MASTER)) return 0;
    if (replyHoldWaitLocalAof()) return 1;
    if (c->reply_hold_chunks &&
        listLength(c->reply_hold_chunks) > 0) return 1;
    return 0;
}

static inline size_t replyHoldChunkOverhead(replyHoldChunk *chunk) {
    return sizeof(listNode) + sizeof(replyHoldChunk) +
           listLength(chunk->reply_list) *
               (sizeof(listNode) + sizeof(clientReplyBlock));
}

static void linkClientToReplyHoldPending(client *c) {
    if (c->reply_hold_pending_node) return;
    listAddNodeTail(server.reply_hold_pending_clients, c);
    c->reply_hold_pending_node = listLast(server.reply_hold_pending_clients);
}

static void unlinkClientFromReplyHoldPending(client *c) {
    if (!c->reply_hold_pending_node) return;
    listDelNode(server.reply_hold_pending_clients, c->reply_hold_pending_node);
    c->reply_hold_pending_node = NULL;
}

/* Free a chunk's contents: releases BULK_STR_REF refcounts and decrements
 * the chunk-side accounting. */
static void freeReplyHoldChunkContents(client *c, replyHoldChunk *chunk) {
    listIter li;
    listNode *ln;
    ...
    c->reply_hold_mem -= chunk->bytes + chunk->overhead;
...
static void replyHoldStart(client *c, size_t *inline_start, listNode **list_tail_start) {
    *inline_start = c->bufpos;
    *list_tail_start = listLast(c->reply);
    c->reply_hold_boundary_node = *list_tail_start;
}

/* Convenience wrapper combining the clientReplyHoldActive() check with
 * replyHoldStart(): every call site needs both together, so this
 * bundles them into one call and one cookie instead of separate active-flag
 * and snapshot locals. */
replyHoldCookie replyHoldBegin(client *c) {
    replyHoldCookie sr = {0, 0, NULL};
    sr.active = clientReplyHoldActive(c);
    if (sr.active)
        replyHoldStart(c, &sr.inline_start, &sr.list_tail_start);
    return sr;
}
 ...
 * c->reply was non-empty and the command might chunk, replyHoldStart
 * captures c->reply_hold_boundary_node so the first _addReplyPayloadToList call
 ...
 * whether to call). An empty reply_list is legal (zero-byte chunks for
 * CLIENT REPLY OFF/SKIP and other empty-reply chunked commands): the chunk's
 * presence at the queue head still gates subsequent replies on its woff.
 *
 * woff is the offset that must be durable before this chunk may drain:
 *   - a write chunk carries the write's replication offset;
...
void replyHoldFinish(client *c, long long woff, const replyHoldCookie *sr) {
    ...
    replyHoldChunk *chunk = zmalloc(sizeof(*chunk));
    chunk->woff = woff;
     ...
     *    Accounting: bytes go directly into c->reply_hold_mem (and the
     *    global counter) — they never lived in c->reply_bytes. */
    if (inline_delta > 0) {
        size_t usable_size;
        clientReplyBlock *block = zmalloc_usable(inline_delta + sizeof(clientReplyBlock), &usable_size);
     ...
     *    c->reply_hold_mem. This keeps the invariant
    ...
    chunk->overhead = replyHoldChunkOverhead(chunk);
    if (c->reply_hold_chunks == NULL)
        c->reply_hold_chunks = listCreate();
    int was_empty = (listLength(c->reply_hold_chunks) == 0);
    listAddNodeTail(c->reply_hold_chunks, chunk);
    server.reply_hold_pending_commands++; /* gauge: one more chunk parked */
    c->reply_hold_mem += chunk->overhead;
    if (was_empty) linkClientToReplyHoldPending(c);
    server.reply_hold_count++;
    /* Depth seen by this command on arrival, including itself (the chunk is
     * already linked above). hold_depth_sum/hold_count gives the saturation-safe
     * "avg commands blocked together". */
    server.reply_hold_depth_sum += listLength(c->reply_hold_chunks);
     ...
     * already accounts for reply_hold_mem. */
    closeClientOnOutputBufferLimitReached(c, 1);
}
 ...
 * existing chunks. Shared tail end of any replyHoldStart/...(work)/
 ...
 * replyHoldFinishOrDefer below for that case). Used directly by callers
...
void replyHoldFinishByOffset(client *c, long long pre_work_repl_offset, const replyHoldCookie *sr) {
    if (server.master_repl_offset > pre_work_repl_offset) {
        ...
        replyHoldFinish(c, c->woff, sr);
    } else if (c->reply_hold_chunks && listLength(c->reply_hold_chunks) > 0) {
...
void replyHoldFinishOrDefer(client *c, const replyHoldCookie *sr) {
    ...
    replyHoldFinishByOffset(c, c->reply_hold_pre_command_repl_offset, sr);
}
 ...
 * released. */
static void replyHoldReleaseChunk(client *c, listNode *ln) {
    replyHoldChunk *chunk = listNodeValue(ln);

    /* Transfer chunk bytes back into the c->reply accounting domain before
     * splicing nodes. Capture the overhead too: it is derived from
     ...
     * c->reply_hold_mem on every drained chunk, slowly inflating
     * mem_clients_normal. */
    if (chunk->bytes > 0) {
        c->reply_hold_mem -= chunk->bytes;
    ...
    server.reply_hold_latency_usec += ustime() - chunk->enqueue_us;

    c->reply_hold_mem -= chunk->overhead;
    listRelease(chunk->reply_list); /* now empty after listJoin */
    zfree(chunk);
    listDelNode(c->reply_hold_chunks, ln);
    server.reply_hold_pending_commands--; /* gauge: one chunk released */

    if (listLength(c->reply_hold_chunks) == 0)
        unlinkClientFromReplyHoldPending(c);
}
...
static void replyHoldArmWriteHandler(client *c) {
 ...
 * the CURRENT config, not captured at enqueue, so switching appendfsync away
 * from bgalways releases whatever is still parked. */
void drainReplyHoldChunks(client *c) {
    if (!c->reply_hold_chunks) return;

    listNode *ln;
    while ((ln = listFirst(c->reply_hold_chunks)) != NULL) {
        replyHoldChunk *chunk = listNodeValue(ln);

        /* Head-of-queue only: a chunk that cannot be released blocks every
         * chunk behind it, or the client would observe replies out of command
         * order. A passthrough chunk (woff 0) always passes this test and so
         * drains together with the chunk it queued behind. */
        if (replyHoldWaitLocalAof() && server.fsynced_reploff < chunk->woff) break;

        replyHoldReleaseChunk(c, ln);
    }

    replyHoldArmWriteHandler(c);
}

/* Tear down every client currently holding pending chunks. Used when their
 * data can never become durable (an AOF write/fsync error) or when the offset
 * they are waiting on stops being meaningful (demotion to replica), where
...
void disconnectAllReplyHoldPendingClients(const char *reason) {
    ...
    if (n > 0) {
        server.reply_hold_pending_disconnects += n;
        serverLog(LL_NOTICE,
            "Disconnecting %d client(s) with pending replies: %s", n, reason);
    }
}
...
void freeReplyHoldChunks(client *c) {
    ...
    listNode *ln;
    ustime_t now = ustime();
    listRewind(c->reply_hold_chunks, &li);
    while ((ln = listNext(&li))) {
        replyHoldChunk *chunk = listNodeValue(ln);
        /* Count the wait time of disconnected pending chunks too so the
         * hold-latency metric isn't undercounted for OBL/error kills. */
        server.reply_hold_latency_usec += now - chunk->enqueue_us;
        freeReplyHoldChunkContents(c, chunk);
        zfree(chunk);
    }
    /* gauge: drop all still-parked chunks at once (O(1), listLength is cached) */
    server.reply_hold_pending_commands -= listLength(c->reply_hold_chunks);
    listRelease(c->reply_hold_chunks);
    c->reply_hold_chunks = NULL;
    c->reply_hold_mem = 0;
    c->reply_hold_boundary_node = NULL;
    unlinkClientFromReplyHoldPending(c);
...
int closeClientOnOutputBufferLimitReached(client *c, int async) {
    ...
    /* "Nothing pending" early-out must include parked chunk bytes, otherwise a
     * client whose only pending bytes live in chunks would never get
     * OBL-disconnected. */
    if ((c->reply_bytes == 0 && c->reply_bytes_shared == 0 &&
         c->reply_hold_mem == 0 && !clientTypeIsSlave(c)) ||
