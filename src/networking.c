...
static inline int _writeToClientSlaveIOThread(client *c, ssize_t *nwritten) {
    ...
    if (pos > c->io_curr_block_pos) {
        ssize_t consumed;
        if (clientCompressesWrites(c)) {
            /* Note, that consumed is how much bytes we've read from the repl
             * buffer, whereas socket_written is the number of compressed bytes
             * actually written to the socket, which would most certainly be a
             * different number. */
            ssize_t socket_written = 0;
            consumed = clientCompressAndWriteBuf(c, o->buf+c->io_curr_block_pos,
                                                 pos-c->io_curr_block_pos,
                                                 &socket_written);
            if (consumed <= 0) {
                if (consumed < 0) *nwritten = -1;
                return C_ERR;
            }
         ...
         * bytes were read from the repl buffer node. */
...
static inline int _writeToClientSlave(client *c, ssize_t *nwritten) {
    *nwritten = 0;
    ...
    if (o->used > c->ref_block_pos) {
        ...
        if (clientCompressesWrites(c)) {
            ssize_t socket_written = 0;
            consumed = clientCompressAndWriteBuf(c, o->buf+c->ref_block_pos,
...
void readQueryFromClient(connection *conn) {
    ...
    if (clientDecompressesReads(c)) {
        /* nread is the number of decompressed bytes placed in the query
         * buffer, whereas network_read is the number of (compressed) bytes
         * actually read from the socket. */
        nread = clientReadAndDecompress(c, c->querybuf+qblen, readlen, &network_read);
    } else {
        nread = connRead(c->conn, c->querybuf+qblen, readlen);
        if (nread > 0) network_read = nread;
    }
    if (nread == -1) {
        /* clientReadAndDecompress sets read_error on a decompression error,
         * in which case the link is not recoverable even if the connection
         * is still alive. */
        if (c->read_error == 0 && connGetState(conn) == CONN_STATE_CONNECTED) {
            goto done;
        } else {
            if (c->read_error == 0)
